 ...
 * itself complete at /api/intake/<slug>/complete, manages the client's own people
 * at /api/intake/<slug>/contacts, and uploads files at /api/intake/<slug>/upload
 * (large files go straight to Railway; see INTAKE_UPLOAD_BASE there). Those routes live on the Sales Toolbox backend and
 ...
 *
 * Only the client's own verbs are forwarded, and only under /api/intake, so the
 * keyed team routes (/api/clients) are not reachable through here.
 */
const SLUG = /^[a-z0-9]+(-[a-z0-9]+)*$/;
const ACTIONS = new Set(["answers", "complete", "upload", "contacts"]);

async function forward(req: NextRequest, segments: string[], method: "GET" | "PUT" | "POST" | "DELETE") {
  ...
  try {
    const upstream = await fetch(`${loeBaseUrl()}/api/intake/${slug}/${action}${req.nextUrl.search || ""}`, {
      method,
      headers: {
