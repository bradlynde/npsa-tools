...
class TestNotifyResend(unittest.TestCase):
    ...
    def test_send_test_notification_posts_resend(self, mock_post: MagicMock) -> None:
        ...
        with patch.dict(os.environ, self._env, clear=False):
            r = notify.send_test_notification_email("Church Scraper")
        self.assertEqual(r, {"ok": True})
        mock_post.assert_called_once()
        call_kw = mock_post.call_args
        self.assertEqual(call_kw[0][0], notify.RESEND_API_URL)
        self.assertIn("Authorization", call_kw[1]["headers"])
        body = call_kw[1]["json"]
        self.assertEqual(body["to"], ["recipient@test.com"])
    ...
    def test_send_run_complete_posts_resend(self, mock_post: MagicMock) -> None:
        ...
        self.assertIn("Church Scraper", body["subject"])
...
class TestRunCompleteAttachment(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.csv = os.path.join(self._tmp, "Maryland_churches_abc123.csv")
    ...
    def test_csv_is_attached(self, mock_post: MagicMock) -> None:
        ...
        self.assertEqual(att["filename"], "Maryland_churches_abc123.csv")
        self.assertEqual(
            base64.b64decode(att["content"]).decode(),
            "first_name,last_name,email\nA,B,a@b.com\n",
        )
        self.assertIn("Attached: Maryland_churches_abc123.csv", payload["html"])
